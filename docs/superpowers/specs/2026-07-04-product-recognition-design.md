...
## Revisi (2026-07-04, setelah implementasi awal)

Setelah Task 1-5 selesai dibangun dan dicoba di device, user minta disederhanakan: **satu foto, bukan pilih mode**. Barcode live-scan (`mobile_scanner`) dihapus total. Perubahan:

- **RecognizeScreen**: tidak lagi ada kamera live barcode scan atau 3 pilihan mode. Cuma 2 tombol seperti `ScanScreen` lama: "Ambil Foto" / "Pilih dari Galeri" (reuse widget `SourceOptionCard`).
- **Satu foto** (label komposisi ATAU kemasan, user tidak perlu tahu bedanya) dikirim ke method baru `GeminiService.analyzePhoto({required File imageFile, required String resultId})` — vision-only, satu prompt yang membaca OCR teks, merek/logo, bentuk kemasan, daftar komposisi (kalau kelihatan), DAN angka barcode tercetak (kalau kelihatan sebagai teks — bukan dekode simbol barcode, itu di luar kemampuan vision LLM yang reliable). Return `sealed class PhotoAnalysisResult` — `DirectAnalysisResult(AnalysisResult result)` kalau komposisi langsung terbaca, atau `NeedsLookupResult(productName, brand, category, barcodeDigits, confidence)` kalau cuma kemasan yang kelihatan.
- `identifyPackaging()` **dihapus** (fungsinya digantikan `analyzePhoto`). `searchCompositionByWeb()`, `analyzeIngredientsFromText()`, `ProductLookupService`, `isCompositionNotFound()` **tetap dipakai**, tidak berubah.
- `RecognizeBloc`: event `BarcodeDetected`+`PackagingPhotoCaptured` diganti satu event `PhotoTaken(File image)`. Alur: `analyzePhoto` → kalau `DirectAnalysisResult` langsung selesai (simpan+sukses) → kalau `NeedsLookupResult`: coba `ProductLookupService.lookupByBarcode(barcodeDigits)` dulu kalau ada angka barcode (kalau gagal/exception, lanjut abaikan, jangan hard-fail — angka hasil OCR bisa salah baca), kalau tidak ketemu baru `searchCompositionByWeb`, hasil composition text apa pun yang didapat lanjut `analyzeIngredientsFromText` (method `_analyzeAndEmit` yang sudah ada dipakai ulang apa adanya).
- State disederhanakan: `RecognizeLookingUp` dan `RecognizeIdentifying` dihapus, diganti satu `RecognizeAnalyzingPhoto` untuk fase awal. `RecognizeSearchingComposition`, `RecognizeAnalyzing`, `RecognizeSuccess`, `RecognizeNotFound`, `RecognizeError` tetap.
- Tidak ada lagi tombol "Foto Komposisi Manual" terpisah — kasus itu sudah otomatis kena jalur `direct_analysis`. Kalau gagal total (tidak kebaca dan tidak ketemu komposisi apa pun), `RecognizeNotFound` cukup menampilkan pesan + tombol ambil ulang.
- Dependency `mobile_scanner` **dihapus** dari `pubspec.yaml`.
- Route `/scan` (`ScanBloc`/`scan_screen.dart`) tetap ada di codebase (tidak dihapus, tidak disentuh) tapi sudah tidak ada link UI ke sana dari `/recognize` lagi — murni jaga-jaga, bukan bagian aktif dari alur baru ini.
